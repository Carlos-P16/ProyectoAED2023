...
En conclusión, el conjunto de variables con las que podemos trabajar de forma simultánea respecto al tiempo son: PM2.5, PM10, NO, NO2, NOx, O3, SO2, CO, C7H8, C6H6, C8H10, Ruido, Velocidad del viento, Direccion del viento, Temperatura, Humedad_relativa, Presion y Radiación solar. Hay que tener en cuenta que la diferencia de NA's entre las variables puede indicar periodos intermedios donde no se han tomado datos, o que en algunas estaciones meteorológicas no se toman esos datos.

```{r}
aux <- pivot_longer(data = datos, cols = 4:ncol(datos), names_to = "Variable", values_to = "Valor") %>% na.omit
aux$Variable <- as.factor(aux$Variable)
ggplot(data = aux, aes(y=Variable, color=Variable)) + 
    geom_point(aes(x=Fecha)) + 
    facet_wrap(~Estacion, nrow=6) + 
