...
NOTA: Mencionar lo dicho en la descripción del dataset sobre la correlación de NOx = (NO,NO2,...)
NOTA: Temperatura y radiación solar
...
datos_ruido_estaciones <- datos_limpios[c("Fecha","Estacion","Ruido")] %>%
  ...
  summarize(Media_Ruido = mean(Ruido, na.rm = TRUE, .groups = "drop")) %>%
  group_by(Ano) %>%
  summarize(Desviacion_por_ano = sd(Media_Ruido))
