...
```{r}
df <- cor(na.omit(datos_clean_outliers[columnas_numericas])) %>%
  as.data.frame(.)
...
correlaciones <- cor[cor$Correlation > 0.7]
correlaciones_inversas <- cor[cor$Correlation < -0.7]

#ggcorr(datos_clean_outliers[columnas_numericas])
```
