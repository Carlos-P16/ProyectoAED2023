...
datos %>% group_by(Estacion) %>% summarise(across(.cols = all_of(variables_numericas), .fns = num_entradas)) %>% pivot_longer(cols = all_of(variables_numericas), names_to = 'Variable', values_to = "Numero_entradas") %>% mutate(Variable = as.factor(Variable)) %>% pivot_wider(names_from = 'Estacion', values_from = 'Numero_entradas')
```

```{r}
datos %>% pivot_longer(cols = all_of(variables_numericas), names_to = 'variables_numericas', values_to = 'valor', values_drop_na = T) %>% ggplot(aes(x=Estacion, fill=variables_numericas)) + geom_bar() + theme(axis.text.x=element_text(angle=45, hjust=1))
```
