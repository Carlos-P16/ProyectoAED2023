...
topCount <- function(x){
  as.character(sum(as.character(x)==names(which.max(table(x))), na.rm = T))
}

topFrec <- function(x){
  as.character(round(sum(as.character(x)==names(which.max(table(x))), na.rm = T)/length(x), digits = 3))
}
...
sumario <- list(type = ~class(.x), levels = ~levels(.x), topLevel = ~topLevel(.x), topCount = ~topCount(.x), topFrec = ~topFrec(.x), missFrec = ~missFrec(.x))

datos %>%
  summarise(across(.cols = all_of(colnames(datos)), .fns = sumario, .names = "{.col}-{.fn}")) %>% 
  pivot_longer(cols = 1:203, names_to = 'variable_estadistica', values_to = 'valor') %>%
  separate(variable_estadistica, into =  c('variable', 'estadistica'), sep = '-') %>%
...
```{r}
datos <- datos %>% select(-c('Id', 'Dia_de_la_semana','Dia_del_mes','Fecha_creacion', 'Fecha_baja', 'B(a)p'))
```
...
datos[c(2,c(9:16))] %>%
  pivot_longer(cols = 2:9, names_to = 'Variables', values_to = 'valor', values_drop_na = T) %>%
  ggplot(aes(x=valor)) + 
  geom_histogram(aes(y=stat(density), fill='blue', alpha=0.6), bins=50) +
  geom_density() +
  facet_wrap(~Variables, scales = "free") + 
